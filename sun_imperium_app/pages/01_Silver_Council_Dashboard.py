...
# NOTE: Streamlit multipage apps normally land on app.py.
# The DM requested that the Silver Council Dashboard be the landing view.
sb = get_supabase()
ensure_bootstrap(sb)
...
eco = (
    ...
    .select("population,survival_ratio,player_payout,tax_income,gross_value")
    .eq("week", week)
    .limit(1)
    .execute()
    .data
...
if eco:
    ...
    st.warning(
        "Economy not computed for this week yet. Use DM Console → Advance Week to generate income."
    )
...
def sum_cat(prefix: str) -> float:
    return sum(
        float(r["amount"])
        for r in wk_rows
        if r["direction"] == "out" and str(r["category"]).startswith(prefix)
    )
...
st.info(
    "Tip: Purchases immediately reduce the Moonvault. Weekly income and upkeep are applied when the week advances."
)
