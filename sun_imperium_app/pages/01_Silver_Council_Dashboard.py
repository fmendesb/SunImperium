...
if week and week > 1:
    prev_eco = (
        ...
        .select("population,survival_ratio")
        .eq("week", week - 1)
        .limit(1)
        .execute()
        .data
...
if eco:
    row = eco[0]
    prev_pop = int(prev_eco[0]["population"]) if prev_eco else None
    e1, e2, e3, e4 = st.columns(4)
    with e1:
        cur_pop = int(row.get("population") or 0)
        delta = (cur_pop - prev_pop) if prev_pop is not None else None
        st.metric("Population", f"{cur_pop:,}", delta=(f"{delta:+,}" if delta is not None else None))
    with e2:
