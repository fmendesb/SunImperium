...
def dm_gate(prompt: str, key: str = "dm", password: str | None = None) -> bool:
    """
    DM gate that works inside or outside st.form().
    No st.button() used (buttons inside forms cause StreamlitAPIException).
    Unlock persists in session_state for this key.
    """
    # Where the password comes from
    expected = password or st.secrets.get("DM_PASSWORD") or st.session_state.get("DM_PASSWORD")

    # If no password configured, treat as locked (safer than open)
    if not expected:
